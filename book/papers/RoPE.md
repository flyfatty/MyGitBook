...
\pmb{g}(\pmb{q}, \pmb{k}, m-n) &= R_{\pmb{g}}(\pmb{q},\pmb{k},m-n)e^{i\theta_{\pmb{g}}(\pmb{q},\pmb{k},m-n)} 
\end{aligned} \tag{2}
$$

如果我们能够求出(2)式中的$$R$$和$$\theta$$，那么我们就可以表示变换f。

将(2)式代入(1.2)式得：
$$
\begin{aligned}
R_{\pmb{f}}(\pmb{q},m)\cdot R_{\pmb{f}}(\pmb{k},n) &= R_{\pmb{g}}(\pmb{q},\pmb{k},m-n) \\
\theta_{\pmb{f}}(\pmb{q},m)-\theta_{\pmb{f}}(\pmb{k},n) &= \theta_{\pmb{g}}(\pmb{q},\pmb{k},m-n) 
\end{aligned} \tag{3}
$$ 
